...
def fetch_checks_for_upload(conn: sqlite3.Connection, upload_id: str) -> list[dict[str, Any]]:
    """Every logged check-step row for one upload, oldest first -- e.g. for a
    batch report that needs each step's detail/tokens/cost/latency alongside
    the final decision (see scripts/batch_check.py). If `upload_id` was
    checked more than once (a --force re-run), later rows for the same
    check_name naturally come after earlier ones; a caller building a
    {check_name: row} dict by iterating this list keeps the most recent.
    """
    rows = conn.execute(
