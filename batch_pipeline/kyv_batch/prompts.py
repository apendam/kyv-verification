...
2. Does the image look altered, AI-generated/synthetic, a screenshot/UI capture, or a \
   ...
   only flag clear cases, not just low quality or an odd angle. Two specific tells worth \
   checking deliberately, not just an overall impression: (a) any visible watermark, \
   logo, or signature mark typical of an AI image generator anywhere in the frame — a \
   small corner mark, a semi-transparent overlay, a repeating tiled pattern, or text like \
   "AI" or "Generated by..."; (b) a sticker or tag on the windshield (e.g. a FASTag) that \
   looks pasted into the photo rather than physically stuck to the glass — noticeably \
   sharper focus, higher resolution, or more saturated colors than the rest of the \
   vehicle, inconsistent lighting or shadow, or edges that don't sit naturally on the \
   glass. Flag either of these as altered even if the rest of the photo looks ordinary.
...
Also assess whether the plate itself looks like a genuine physical plate rather than a \
digitally altered or composited one. Look specifically for: text that looks pasted onto \
the plate rather than part of its surface; a character layout inconsistent with the \
plate's own visible proportions or mounting slot (e.g. a single flattened line where the \
slot/frame implies two); lighting or reflection on the plate that doesn't match the rest \
of the vehicle; or a missing element the plate's evident format should have (e.g. the IND \
hologram mark on a high-security plate). Set plate_looks_physically_genuine to false ONLY \
when you can point to one of these specific signs — never from a hunch, low image \
quality, or normal wear.
